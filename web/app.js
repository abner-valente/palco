...
/* ----------------------------------------------------------------
   Configuracao basica (espelha as constantes do main.py / pieces.py)
   ---------------------------------------------------------------- */
...
const CONNECTION_CLICK_WIDTH = 9; // espessura visivel da linha (combina com pieces.py)
...
let nextId = 0;
const pieces = new Map(); // id -> {id, shape, size, x, y, radius, label, el, imgEl, labelEl}
const bases = new Map();  // id -> {id, color, x, y, radius, el}
const connections = new Map(); // id -> {id, aId, bId, el}

let draggingObj = null;       // { kind: 'piece'|'base', id }
let draggingNewItem = null;   // { kind, shape/color, size, ghostEl }
...
const colPieces = document.getElementById("col-pieces");
const colBases = document.getElementById("col-bases");
...
function makePieceVisual(shape) {
  ...
  img.onerror = () => {
    holder.innerHTML = "";
    holder.appendChild(buildShapeFallback(shape));
  };
...
function startPaletteDrag(evt, item) {
  ...
  const up = (e) => {
    ...
    if (e.clientX >= rect.left && e.clientX <= rect.right && e.clientY >= rect.top && e.clientY <= rect.bottom) {
      ...
      if (draggingNewItem.kind === "piece") {
        createPiece(draggingNewItem.shape, draggingNewItem.size, point.x, point.y);
      } else {
        createBase(draggingNewItem.color, point.x, point.y);
      }
    }
...
function createPiece(shape, size, x, y, options = {}) {
  ...
  const visual = makePieceVisual(shape);
...
function updateTrashHighlight(evt) {
  if (draggingObj && trashRectContains(evt)) {
    trashEl.classList.add("highlight");
  } else {
...
function refreshConnectionLine(conn) {
  const a = pieces.get(conn.aId);
  const b = pieces.get(conn.bId);
  if (!a || !b) return;
  conn.el.setAttribute("x1", a.x);
  conn.el.setAttribute("y1", a.y);
  conn.el.setAttribute("x2", b.x);
  conn.el.setAttribute("y2", b.y);
}
...
/* ----------------------------------------------------------------
   Tecla Delete: remove peca/base sob o mouse, ou a linha selecionada
   ---------------------------------------------------------------- */

let lastPointerPos = { x: -1, y: -1 };
stageWrapper.addEventListener("pointermove", (evt) => {
  lastPointerPos = stagePointFromEvent(evt);
});
...
function findPieceAt(point) {
  for (const piece of pieces.values()) {
    ...
    if (Math.hypot(dx, dy) <= piece.radius) return piece;
  }
  return null;
}
...
function findBaseAt(point) {
  for (const base of bases.values()) {
    ...
    const dy = (point.y - base.y) * 2; // base e mais achatada (elipse)
...
/* Clicar em area vazia do palco desmarca a linha selecionada */
...
/* ----------------------------------------------------------------
   Salvar / Carregar sessao (.json local)
   ---------------------------------------------------------------- */

function serializeSession() {
  return {
    pieces: [...pieces.values()].map((p) => ({
      id: p.id, shape: p.shape, size: p.size, x: p.x, y: p.y, label: p.label, radius: p.radius,
    })),
    bases: [...bases.values()].map((b) => ({
      id: b.id, color: b.color, x: b.x, y: b.y, radius: b.radius,
    })),
...
document.getElementById("load-input").addEventListener("change", (evt) => {
  ...
  reader.onload = () => {
    try {
      ...
      alert("Arquivo de sessao invalido.");
    }
  };
